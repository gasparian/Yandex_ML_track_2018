...
path = config.path +'/data/svd_cv_features_svd_%s' % config.svd_n_components
try:
    os.mkdir(path)
except:
    pass
...
for train_index, test_index in splits:
    ...
    train_data = np.hstack([models[fold].transform(train[str(col)].loc[train_index]) for col in config.text_cols])
    test_data = np.hstack([models[fold].transform(train[str(col)].loc[test_index]) for col in config.text_cols])
    hold_out_test_data = np.hstack([models[fold].transform(test[str(col)]) for col in config.text_cols])
...
logging.info('SVD preparation finished!')