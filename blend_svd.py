...
for fold in range(nfolds):
    ...
    if models[fold]['best'] == 'averaged':
        if model[fold]['mode'] == 'mean':
            scores = np.ones((hold_out_test_data.shape[0]))
            for key in model[fold]['model'].keys():
                scores += model[fold]['model'][key].predict(hold_out_test_data) / len(model[fold]['model'])
        elif model[fold]['mode'] == 'geom':
            scores = np.ones((hold_out_test_data.shape[0]))
            for key in model[fold]['model'].keys():
                scores *= model[fold]['model'][key].predict(hold_out_test_data)
            scores = np.power(np.abs(scores), (1/len(model[fold]['model'])))
        elif model[fold]['mode'] == 'harmonic':
            scores = np.zeros((hold_out_test_data.shape[0]))
            for key in model[fold]['model'].keys():
                    scores += 1./model[fold]['model'][key].predict(hold_out_test_data)
            scores = len(model[fold]['model'])/scores
    else:
        scores = model[fold]['model'][model['best']].predict(hold_out_test_data)
...
logging.info('Models and submission saved!')